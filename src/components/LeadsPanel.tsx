...
export default function LeadsPanel({
  ...
  if (plan === "free") {
    return (
      <section className="rounded-[1.75rem] border border-black/10 bg-white p-7 shadow-[0_30px_60px_-30px_rgba(14,10,27,0.25)]">
        ...
        <div className="mt-6 rounded-2xl border border-black/10 bg-black/[0.02] px-5 py-5">
          <p className="text-xs font-medium tracking-[0.14em] text-flex-black/45 uppercase">
            Premium
          </p>
          <p className="mt-2 text-sm text-flex-black/65">
  ...
  return (
    <section className="rounded-[1.75rem] border border-black/10 bg-white p-7 shadow-[0_30px_60px_-30px_rgba(14,10,27,0.25)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {leads.length > 0 && (
          <a
            ...
            className="flex items-center gap-1.5 rounded-xl border border-black/12 px-4 py-2 text-[11px] font-semibold tracking-[0.14em] uppercase transition-colors hover:bg-black/[0.03]"
          >
      ...
      {leads.length === 0 ? (
        <p className="mt-6 rounded-2xl border border-dashed border-black/12 px-4 py-6 text-center text-sm text-flex-black/45">
        ...
        <ul className="mt-6 divide-y divide-black/5">
          {leads.map((lead) => (
            <li key={lead.id} className="py-3.5">
              ...
              {lead.company && (
                <p className="mt-0.5 text-sm text-flex-black/50">{lead.company}</p>
              )}
              ...
              <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 font-tabular text-sm">
                {lead.phone && (
                  <a
                    ...
                    className="text-flex-black/75 underline-offset-2 hover:underline"
                  >
                    {lead.phone}
                  </a>
                )}
              ...
              {lead.note && (
                <p className="mt-1.5 text-sm break-words whitespace-pre-wrap text-flex-black/60">
                  {lead.note}
                </p>
              )}
            </li>
