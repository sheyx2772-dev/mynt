  ...
  return (
    <section className="rounded-3xl border border-ink-line bg-ink-s1 p-6 sm:p-8">
      <div className="flex items-start gap-3">
        <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ink-s2">
          <ArrowRightLeft className="h-4 w-4 text-paper-2" />
        </span>
        <div>
      ...
      {live ? (
        <div className="mt-6 rounded-2xl border border-ink-line bg-ink-s2 p-5">
          ...
          <button
            ...
            className="mt-4 rounded-full border border-ink-line px-5 py-2 text-sm font-medium transition-colors hover:bg-ink-s2 disabled:opacity-50"
          >
        ...
        <form action={action} className="mt-6 flex flex-wrap items-start gap-3">
          ...
          <input
            ...
            className="min-w-0 flex-1 rounded-full border border-ink-line px-5 py-2.5 text-sm outline-none placeholder:text-paper-3 focus:border-flex-black/40"
          />
          <button
            type="submit"
            disabled={sending}
      ...
      <p className="mt-4 text-xs leading-relaxed text-paper-3">
        ...
        <ul className="mt-6 space-y-2 border-t border-ink-line pt-5">
          {past.map((t) => (
            <li key={t.id} className="flex items-center justify-between gap-4 text-xs">
              <span className="truncate text-paper-2">{t.toEmail}</span>
              <span className="shrink-0 text-paper-3">
