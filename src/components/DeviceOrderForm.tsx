...
const button =
  "flex items-center justify-center gap-2 rounded-2xl bg-ink-s2 px-6 py-3.5 text-sm font-medium text-paper transition-transform hover:scale-[1.01] disabled:opacity-50";
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <section className="rounded-3xl border border-ink-line bg-ink-s1 p-6 sm:p-8">
        ...
        <p className="flex items-center gap-2 text-sm text-paper-2">
        ...
        {state.error && (
          <p className="mt-4 text-sm text-danger-ink">{state.error}</p>
        )}
